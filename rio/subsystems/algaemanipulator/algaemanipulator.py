...
class AlgaeManipulator(Subsystem):
    ...
    def stop(self, motor: TalonSRX = None) -> Command:
        """STOP"""
        if motor is None:
            return self.run(lambda: self.setSpeed(self.pivotMotor, 0.0)).alongWith(
                lambda: self.setSpeed(self.wheelMotor, 0.0)
            )
        else:
    ...
    def outtake(self) -> Command:  # down = false/none, up = true
        return (
            self.run(lambda: self.setSpeed(self.wheelMotor, -0.60))
            .andThen(WaitCommand(1.5))
            .andThen(lambda: self.setSpeed(self.wheelMotor, 0.0))
        )
    ...
    def pivotStall(self):
        return self.pivotStallDebouncer.calculate(
            abs(self.pivotMotor.getStatorCurrent()) > 20
        )  # TODO: test to figure out current number

    def wheelStall(self):
        return self.wheelStallDebouncer.calculate(
            abs(self.wheelMotor.getStatorCurrent()) > 20
        )  # TODO: test to figure out current number
    ...
    def AlgaeGrabberSequence(self):
        return SequentialCommandGroup(
            self.pivotPosition(True).withTimeout(1).andThen(self.intake())
        )
    ...
    def AlgaeScoringSequence(self):
        return SequentialCommandGroup(
            self.outtake()
            .withTimeout(1.75)
            .andThen(self.wheelStop().andThen(self.pivotPosition(False)))
        )
